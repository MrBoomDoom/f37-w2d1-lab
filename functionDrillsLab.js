...
/*
  ...
  value of the name parameter being passed in).Create a variable called 'nameGreeting' and 
...
function faveColorFinder(color) {
  if(color === 'red') {
    ...
    return `you need to evaluate your fav color choice`
  }
}
...
function chocolateFrogLover(gold){
  return gold / 3
}
let totalFrogs = chocolateFrogLover(28)
...
function chocolateFrogLover2(gold) {
  if (gold % 3 === 0) {
    return gold / 3
  } else if ((gold - 1) % 3 === 0) {
    return (gold - 1) / 3
...
let totalFrogs2 = chocolateFrogLover2(28)
...
for(let i = 0; i < arr.length; i++){
  if(comparisonValue <= arr[i + 1]){
    comparisonValue = arr[i + 1]
  } else {
    return false
  }
}
